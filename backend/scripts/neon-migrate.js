...
async function main() {
  ...
  const deploySuccess = await runWithRetry(`npx prisma migrate deploy --preview-feature --skip-generate`, 'Migration deploy');
  ...
  const resetSuccess = await runWithRetry(`npx prisma migrate reset --force --skip-seed --skip-generate`, 'Migration reset');
